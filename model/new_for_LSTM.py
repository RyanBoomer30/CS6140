...
def run_lstm(activation, optimizer, title_suffix):

    model_all = Sequential([
        LSTM(50, activation=activation, input_shape=(sequence_length, X_train_all.shape[2])),
        Dense(1)
    ])
    ...
    model_limited = Sequential([
        LSTM(50, activation=activation, input_shape=(sequence_length, 1)),
        Dense(1)
    ])
    ...
    plt.title(f'Comparison of Actual vs Predicted Close Prices for LSTM ({title_suffix})')
    plt.legend()
    plt.grid(True)

    plot_filename = os.path.join(output_dir, f"LSTM_{title_suffix.replace(': ', '_').replace(', ', '_').replace(' ', '_')}.png")
...
for activation, optimizer, title_suffix in configurations:
    run_lstm(activation, optimizer, title_suffix)